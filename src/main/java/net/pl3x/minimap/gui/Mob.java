...
public enum Mob {
    ...
    private static final Map<EntityType<? extends MobEntity>, Mob> BY_TYPE = new HashMap<>();

    public Mob byType(EntityType<? extends MobEntity> mob) {
        return BY_TYPE.get(mob);
    }

    static {
        for (Mob marker : values()) {
            BY_TYPE.put(marker.mob, marker);
        }
    }
}
